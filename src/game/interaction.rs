...
impl Plugin for InteractionPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(ActiveTool::Hoe)
            ...
            .add_systems(Update, grow_plants.run_if(in_state(GameState::Playing)));
    }
}
...
fn grow_plants(
    ...
    // Step #1: Cache the next item that should be updated instead of doing this every frame, mark cache as dirty when timers are changed
